...
as $$
  ...
  current_plan as (
    ...
    where bp.building_id = p_building_id and bp.plan_year = p_plan_year
    limit 1
  ),
  upcoming_plan as (
    select bp.currency, bp.monthly_operating_budget
  ...
  current_common_water_bill as (
    ...
    join current_source_billing_period bp on bp.id = b.billing_period_id
  ...
  upcoming_common_water_bill as (
    select to_jsonb(b) as row
    from public.tb810_utility_bills b
    join upcoming_source_billing_period bp on bp.id = b.billing_period_id
    where b.building_id = p_building_id
      and b.utility_type_id = (select id from public.tb810_utility_types where code = 'common_water' limit 1)
    limit 1
  ),
  unit_rows as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'id', u.id, 'unit_number', u.unit_number, 'unit_type_id', u.unit_type_id,
      'unit_type_code', ut.code, 'has_meter', u.has_meter, 'has_gas_service', u.has_gas_service,
  ...
  current_water_readings as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'unit_id', r.unit_id, 'reading_end', r.reading_end, 'consumption', r.consumption,
      'reading_date', r.reading_date, 'created_at', r.created_at
  ...
  gas_bills as (
    ...
    from public.tb810_gas_bills gb where gb.building_id = p_building_id
  ),
  current_gas_readings as (
    ...
    from public.tb810_gas_readings gr where gr.building_id = p_building_id and gr.reading_month = (select current_reading_month from reading_months)
  ),
  upcoming_gas_readings as (
    select coalesce(jsonb_agg(to_jsonb(gr) order by gr.reading_month desc, gr.created_at desc), '[]'::jsonb) as rows
    from public.tb810_gas_readings gr where gr.building_id = p_building_id and gr.reading_month = (select upcoming_reading_month from reading_months)
  ),
  charges as (
    select coalesce(jsonb_agg(to_jsonb(c) order by c.created_at, c.id), '[]'::jsonb) as rows
    from public.tb810_charges c where c.building_id = p_building_id
  )
  select jsonb_build_object(
    ...
    'current', jsonb_build_object(
      ...
      'obligationLifecycle', coalesce((select jsonb_build_object('mode', 'snapshotted', 'billingPeriodId', id, 'billingPeriodStatus', status) from current_snapshot), jsonb_build_object('mode', 'live', 'billingPeriodId', null, 'billingPeriodStatus', null)),
    ...
    'upcoming', jsonb_build_object(
      ...
      'obligationLifecycle', coalesce((select jsonb_build_object('mode', 'snapshotted', 'billingPeriodId', id, 'billingPeriodStatus', status) from upcoming_snapshot), jsonb_build_object('mode', 'live', 'billingPeriodId', null, 'billingPeriodStatus', null)),
