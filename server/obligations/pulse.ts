...
export async function runMonthlyObligationPulse(): Promise<MonthlyObligationPulseResult> {
  ...
  if (periodResult.data && isSnapshotProgressedStatus(String(periodResult.data.status))) {
    return { status: "already_progressed", buildingId: building.id, obligationMonth, billingPeriodId: String(periodResult.data.id), billingPeriodStatus: String(periodResult.data.status) };
  }

  const snapshotResult = await createMonthlyObligationSnapshot({ buildingId: building.id, buildingName: building.name, obligationMonth });
  return mapSnapshotResult({ buildingId: building.id, obligationMonth, result: snapshotResult });
}
