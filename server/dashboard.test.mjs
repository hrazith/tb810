...
function buildProjectionFacts(overrides = {}) {
  const overrideOperating = overrides.operating ?? {};
  ...
  const operating = {
    obligations: {
      ...
      components: {
        ...
        other_charge: { state: "available", amount: "0.00", count: 0 },
        owner_direct_charge: { state: "available", amount: "0.00", count: 0 },
        ...(overrideOperating.obligations?.components ?? {}),
      },
    },
    water: {
      ...
      ...(overrideOperating.water ?? {}),
    },
    gas: {
      supplierBillCount: 0,
      gasReadingCount: 0,
      gasUnitCount: 0,
      ...(overrideOperating.gas ?? {}),
    },
    charges: {
      unitChargeCount: 0,
      ownerDirectChargeCount: 0,
      ...(overrideOperating.charges ?? {}),
    },
  };
...
test("K dashboard facts read uses exactly two bounded month reads", async () => {
  ...
  ownerFactsModule.loadBuildingMonthFinancialFacts = async ({ obligationMonth }) => {
    ...
    const commonWaterBill = obligationMonth === "2026-08"
      ? {
          id: "water-aug",
          amount: "125.50",
    ...
    const data = {
      obligationMonth,
      sourceReadingMonth: obligationMonth === "2026-08" ? "2026-07" : "2026-08",
      planYear: 2026,
      ...
      waterReadings: [
        {
          ...
          reading_end: 100,
          consumption: 10,
          reading_date: obligationMonth === "2026-08" ? "2026-07-28" : "2026-08-28",
          created_at: "2026-08-28T00:00:00Z",
        },
      ],
      gasBills: obligationMonth === "2026-08"
        ? [{ id: "gas-1", amount: 50, processed_at: null, invoice_date: "2026-08-19" }]
        : [
            { id: "gas-2", amount: 75, processed_at: null, invoice_date: "2026-09-02" },
            { id: "gas-3", amount: 25, processed_at: null, invoice_date: "2026-09-03" },
          ],
      gasReadings: [
        { unit_id: "u-1", reading_month: obligationMonth === "2026-08" ? "2026-07" : "2026-08", current_reading: 120, previous_reading: 100, consumption: 20 },
      ],
      charges: obligationMonth === "2026-08"
        ? []
        : [
            {
              id: "charge-sep",
  ...
  try {
    ...
    assert.equal(obligationMonths.length, 2);
    assert.deepEqual(obligationMonths, ["2026-08", "2026-09"]);
